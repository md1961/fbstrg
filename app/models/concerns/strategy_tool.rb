module StrategyTool
  ...
  def tries_fourth_down_gamble?(game)
    ...
    return false if game.quarter == 4 && game.time_left >= 2 * 60 && game.yard_to_go >= 10 && !zone_aggresive?(game) && game.timeout_left >= 3
    ...
    game.quarter == 4 && (
       ...
       || (game.score_diff < -14 && zone_aggresive?(game))
    ) \
  ...
  def needs_offense_timeout?(game)
    ...
    return false if game.quarter == 2 && game.down >= 3 && game.yard_to_go >= 15 && game.time_left >= 60
  ...
  def needs_defense_timeout?(game)
    ...
    return false if [1, 3].include?(game.quarter) || game.time_left >= 60 * 4
    game.quarter == 4 && (
         (game.score_diff > 8 && game.time_left <= 60 * 4) \
      || (game.score_diff > 0 && game.time_left <= 60 * 3)
    ) ||
    game.quarter == 2 && (
