module StrategyTool
  ...
  def tries_fourth_down_gamble?(game)
    ...
    return false if game.quarter == 4 && game.score_diff >= -14 && game.ball_on < 40 && game.yard_to_go > 10 && game.time_left >= 10 * 60
  ...
  def needs_offense_timeout?(game)
    ...
    return false if game.quarter == 4 && game.score_diff > 0
    return false if game.quarter == 2 && !zone_aggresive?(game)
  ...
  def kneel_down_to_finish_half?(game)
    game.quarter == 2 && game.score_diff >= -14 && game.ball_on > 2 && game.ball_on <= 30 \
